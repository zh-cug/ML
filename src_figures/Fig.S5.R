...
for (i in 1:6){
  for (j in 1:3){
    ...
    data$model<-models[i]
    data$var<-vars[j]
    figdata[[(i-1)*6+j]]<-data
    print(j)
  }
  print(models[i])
}
figdata<-do.call(rbind, figdata)  

a<-ggplot(subset(figdata, var=="LT"), aes(date, mean, colour=model))+geom_line()+
...
b<-ggplot(subset(figdata, var=="EMI"), aes(date, mean, colour=model))+geom_line()+
  geom_ribbon(aes(ymax=up, ymin=low, fill=model, colour=model), alpha=0.4, colour=NA)+
  theme_bw()+
...
export::graph2jpg(file="Fig.S5.jpg", width=9/2.54, height=18/2.54)
