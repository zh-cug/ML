...
fig.s3<-merge(subset(mp, model!="rf_nt"), site[, c("city", "lon", "lat")], by="city")

ggplot()+
  geom_polygon(data=china, aes(long, lat, group=group), fill="white", colour="black", linewidth=0.3)+
  geom_point(data=fig.s3,
             aes(lon, lat, fill=IOA), shape=21, colour="gray", stroke=0.3)+
...
export::graph2jpg(file="./Figs/Fig.S3_.jpg", width=19/2.54, height=14/2.54)
